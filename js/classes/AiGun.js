...
function AiGun(_type, _x, _y) {
	if (_type===undefined || _x===undefined || _y===undefined) {
		return false;
	}
	this.type=_type;
	...
	// Extent sprite
	Sprite.call(this, pImg+'BuildingGun'+this.type+'.png', 4, _x, _y, -45-Math.random()*90,_offset);
	...
	this.update = function() {
		...
		if (updateObjects.onRunning[this.targetId]) {
			...
			var tDir=Math.atan2(t.y-this.y,t.x-this.x)/Math.PI*180;
			var relDir=tDir-this.dir+360;
			...
			if (gameTime>this.loadedAfter) {
				if (Math.abs(relDir)<this.rotSpeed) {
					...
					if (this.type<4) {
						...
						new GunShot(this.type,this.dir-this.spread/2+Math.random()*this.spread,this.x,this.y);
					} else {
						// The laser gun has a special shot that hits instantly

						// Make laser beam on rock
						var lx=t.x+0*Math.cos(tDir/180*Math.PI+Math.PI);
						var ly=t.y+0*Math.sin(tDir/180*Math.PI+Math.PI);
						...
						var lx=this.x+24*Math.cos(tDir/180*Math.PI);
						var ly=this.y+24*Math.sin(tDir/180*Math.PI);
