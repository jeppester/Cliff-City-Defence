...
Loader.prototype.getImage = function (source) {
	return this.getRessource(source, 'images');
};
...
Loader.prototype.getMusic = function (source) {
	return this.getRessource(source, 'music');
};

Loader.prototype.getRessource = function (source, typeString) {
	var res, inh, i;

	if (source.indexOf('/') !== -1) {
		console.log('Fixing source format: ' + source);
	...
	if (!res) {
		for (i = 0; i < this.themes[engine.theme].inherit.length; i ++) {
			inh = this.themes[engine.theme].inherit[i];
			if (this.themes[inh]) {
				try {
					eval('var res = this.themes.' + inh + '.' + typeString + '.' + source);
				} catch (e) {}
...
Loader.prototype.getImageSources = function () {
	...
	object = this.themes[engine.theme].images;
	sourceStrings = [];
	currentDir = [];
	...
	for (i = 0; i < this.themes[engine.theme].inherit.length; i ++) {
		inheritTheme = this.themes[this.themes[engine.theme].inherit[i]];
...
Loader.prototype.loadClasses = function (_paths) {
	var className, i;

	for (i in _paths) {
		if (_paths.hasOwnProperty(i)) {
			// Check that the class is not already loaded
			className = _paths[i].match(/(\w*)\.\w+$/)[1];
			if (window[className]) {continue; }

			jseSyncLoad(_paths[i]);
			this.loaded.classes[className] = _paths[i];
		}
	}
	return true;
};
...
Loader.prototype.loadThemes = function (themeNames) {
	var name, req, i;
