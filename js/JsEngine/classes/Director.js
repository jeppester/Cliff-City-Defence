/*
Director:
The director handles scene with animation of sprites.
A scene is a json object consisting of a list of actors (sprites) and a timeline with their movements.
...
jseCreateClass('Director');
jseExtend(Director, ObjectContainer);
...
Director.prototype.createActors = function () {
	...
	for (name in this.scene.actors) {
		if (this.scene.actors.hasOwnProperty(name)) {
			...
			this.children = jseArraySortByNumericProperty(this.children, 'drawOrder');
		}
	}
};
...
Director.prototype.pauseScene = function () {
	engine.removeActivityFromLoop(this, this.onRunning, 'eachFrame');
};

Director.prototype.playScene = function () {
	engine.addActivityToLoop(this, this.onRunning, 'eachFrame');
};

Director.prototype.goToFrame = function (frame) {
	this.frame = frame;
	this.updateScene();
};
...
Director.prototype.getKeyFrameProperties = function (actor, keyFrameNumber) {
	...
	for (propName in ret) {
		if (ret.hasOwnProperty(propName)) {
			frame = jseArrayElementByPropertyValue(actor.keyFrames, 'frame', keyFrameNumber);
...
Director.prototype.onRunning = function () {
	...
	if (this.frame === this.scene.duration) {
		engine.removeActivityFromLoop(this, this.updateScene, 'eachFrame');
		this.frame = 0;
	}
	this.updateScene();
};

Director.prototype.actorGetFrame = function (actor, frame) {
	var keyFrame, i;
	for (i = 0; actor.keyFrames.length; i ++) {
		keyFrame = actor.keyFrames[i];
